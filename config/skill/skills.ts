import { Skill } from "./skills.type";

export const NextJS: Skill = {name: "NextJS", pathImage: "/icon/nextjs.png", url: "https://nextjs.org/"};
export const ReactJS: Skill = {name: "ReactJS", pathImage: "/icon/react.png", url: "https://reactjs.org/"};
export const TypeScript: Skill = {name: "TypeScript", pathImage: "/icon/typescript.png", url: "https://www.typescriptlang.org/"};
export const TailwindCSS: Skill = {name: "TailwindCSS", pathImage: "/icon/tailwindcss.png", url: "https://tailwindcss.com/"};
export const Vercel: Skill = {name: "Vercel", pathImage: "/icon/vercel.png", url: "https://vercel.com/"};
export const Rust: Skill = {name: "Rust", pathImage: "/icon/rust.png", url: "https://www.rust-lang.org/"};